...
module.exports = function (app) {
    ...
    app.get('/cart', function (req, res) {
        var cartItems = req.session.item;
        var totalPrice = 0;
        Good.bulkQuery(Object.keys(cartItems), function (err, goods) {
            if (err) {
                return console.log(err);
            }
            for(var i=0 ;i<goods.length;i++){
                goods[i].count = cartItems[goods[i]._id];
                totalPrice +=goods[i].count * goods[i].price;
            }
            res.render('cart', {
                title: "购物车",
    ...
    app.post('/addcart', function (req, res) {
        ...
        var itemId = req.body.good_id;
        if (type == "add") {
            if(!goods[itemId]){
                goods[itemId] = 1;
            }else{
                goods[itemId] = parseInt(goods[itemId]) + 1;
            }
            req.session.total += 1;
        }
        if (type == "minus") {
    ...
    app.get('/addGoodAttr', function (req, res) {
        res.render('adminpage/addGoodAttr', {
            title: "pos机后台管理系统",
            success: req.flash('success').toString(),
            error: req.flash('error').toString()
        });
    ...
    app.get('/delAttr1', function (req, res) {
        res.render('adminpage/deleAttrFromAdd', {
            title: "pos机后台管理系统",
    ...
    app.get('/delAttr2', function (req, res) {
        ...
        Good.get_good(goodId, function (err, attr) {
            ...
            res.render('adminpage/deleAttrFromDet', {
                title: "pos机后台管理系统",
    ...
    app.get('/gooddetail', function (req, res) {
        ...
        Good.get_good(goodId, function (err, good) {
            ...
            res.render('adminpage/goodDetail', {
                title: "pos机后台管理",
                success: req.flash('success').toString(),
                error: req.flash('error').toString(),
                good: good[0]
    ...
    app.get('/addpromotion', function (req, res) {
        res.render('promotionpage/addpromotion', {
            success: req.flash('success').toString(),
            error: req.flash('error').toString()
        });
    });
    ...
    app.get('/addrule', function (req, res) {
        res.render('promotionpage/addrule', {
            success: req.flash('success').toString(),
            error: req.flash('error').toString()
        });
    });
